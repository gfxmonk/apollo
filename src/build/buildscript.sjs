...
function build_deps() {
  ...
  BUILD("test/_index.txt",
      ...
      "echo '### regenerate it by running src/build/make-apollo' >> $TARGET",
  ...
  BUILD(
    ...
    function() {
      ...
      if (process.env['APOLLO_MINIFY'] == 'false') {
        ...
        //var c = require('../../modules/compile/minify.sjs'); // TODO
...
function STRINGIFY(target, source, flags) {
  ...
  BUILD(
    ...
    function() {
      ...
      var src = fs.readFile(source).toString();
      //var c = require('../../modules/compile/stringify.sjs'); // TODO
